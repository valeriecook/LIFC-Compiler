...
char *smap_get_key(smap *map, int i) {
    for (int j = 0; j < map->num_buckets; j++) {
	if (i < map->buckets[j].num_pairs) {
	    return map->buckets[j].pairs[i].key;
	} else {
	    i -= map->buckets[j].num_pairs;
	}
    }
    return NULL;
}

int smap_num_pairs(smap *map) {
